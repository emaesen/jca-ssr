 ...
 * https://github.com/lukaszflorczak/vue-agile/blob/master/src/mixins/preparations.js
 * 2019-05-26
 */
const mixin = {
	methods: {
		...
		prepareSlidesClasses () {
			...
			for (let i = 0; i < this.slidesCount; i++) {
				this.slides[i].classList.remove('carousel__slide--active')
				this.slides[i].classList.remove('carousel__slide--current')
			}
			...
			for (let i = Math.max(start, 0); i < Math.min(start + this.settings.slidesToShow, this.slidesCount); i++) {
				this.allSlides[i].classList.add('carousel__slide--current')
			}
		},
		 ...
		 */
		prepareCarousel () {
			this.widthSlide = !this.settings.uncarousel ? this.widthContainer / this.settings.slidesToShow : 'auto'
